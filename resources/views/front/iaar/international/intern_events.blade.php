        ...
        <div class="container">
            <div class="page_sidebar">
                <ul class="page_nav">
                    ...
                    <li><a class="nav_item" href="/iaar/{{\App\Models\StaticPage::where('type_id',22)->first()->slug}}/{{$lang}}">{{\App\Models\StaticPage::where('type_id',22)->first()->getLocaleNode($lang)->title}}</a></li>
                    <li><a class="nav_item active" href="/iaar/{{\App\Models\StaticPage::where('type_id',23)->first()->slug}}/{{$lang}}">{{\App\Models\StaticPage::where('type_id',23)->first()->getLocaleNode($lang)->title}}</a></li>
                </ul>
            ...
            <div class="structure_block">
                ...
                <div class="mezh_events">
                    @foreach($events as $event)
                    <div class="mezh_event_item">
                        <a class="mezh_event_img" style="background-image: url('{{$event->main_image}}');" href="/iaar/international_events/{{$event->slug}}/{{$lang}}" ></a>
                        <div class="mezh_event_text">
                            <a href="/iaar/international_events/{{$event->slug}}/{{$lang}}" class="sidebar_title event_name">{{$event->getLocaleNode($lang)->title}}</a>
                            <p class="text">{{$event->getLocaleNode($lang)->additional_documents}}</p>
                            <a href="/iaar/international_events/{{$event->slug}}/{{$lang}}" class="sovet_btn">{{__('main.More')}}</a>
                        </div>
                    </div>
                    @endforeach
                </div>
