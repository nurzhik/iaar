...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление новой страницы к категории "{{\App\Models\StaticPage::getRussianName($category)}}"
            @else
                Редактирование страницы  "{{$item->title}}" категории "{{\App\Models\StaticPage::getRussianName($category)}}"
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/static/{{$category}}/page/create" @else action="/admin/static/{{$category}}/page/{{$item->id}}" @endif>
            {{csrf_field()}}
                    ...
                    @if(!empty($item->id))
                        <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                            ...
                            @if(!($item->nodes()->where('lang','kz')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_static_page_node',['lang' =>'kz','category' => $category,'parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                            @endif
                            @if(!($item->nodes()->where('lang','en')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_static_page_node',['lang' =>'en','category' => $category, 'parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                            @endif
                            <table class="table table-bordered">
                                ...
                                @foreach($item->nodes()->get() as  $node)
                                    <tr class="success">
                                        ...
                                        <td>{{$node->lang}}</td>
                                        <td><a href="{{route('edit_static_page_node',['lang'=>$node->lang,'category' => $category, 'parent' => $item->id])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_node" data-toggle="modal" data-target="#delete-node-modal" itemname="{{$node->lang}}" itemid="{{$node->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
