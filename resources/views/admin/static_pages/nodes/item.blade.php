...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
            ...
            @else
                Редактирование страницы  переводов на язык "{{$lang}}" "{{$parent->title}}" категории "{{\App\Models\StaticPage::getRussianName($category)}}"
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/static/{{$category}}/page/create" @else action="/admin/static/{{$category}}/nodes/page/{{$parent->id}}/{{$lang}}" @endif>
            {{csrf_field()}}
