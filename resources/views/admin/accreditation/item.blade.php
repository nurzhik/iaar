...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление новой страницы аккредитации к типу учреждений "{{array_flip(\App\Models\Univer::availableTypeIdArray())[$type]}}"
            @else
                Редактирование страницы  аккредитации "{{$item->title}}"  типа учреждений ""{{array_flip(\App\Models\Univer::availableTypeIdArray())[$type]}}""
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/accreditation/{{$type}}/create" @else action="/admin/accreditation/{{$type}}/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                ...
                <ul id="myTabs" class="nav nav-tabs" role="tablist">
                    ...
                    <li role="presentation" class="">
                        <a href="#add_documents" id="add_documents-tab" role="tab" data-toggle="tab" aria-controls="add_documents" aria-expanded="false">Стандарты</a>
                    </li>
                    <li role="presentation" class="">
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    <div role="tabpanel" class="tab-pane fade active in" id="main" aria-labelledby="main-tab">
                        ...
                        <div class="form-group">
                            <label for="sort_order">Тип аккредитациии:</label>
                            <select class="form-control" name="sort_order" id="sort_order">
                                <option @if($item->sort_order == 1 )selected @endif value="1">Институциональная аккредитация</option>
                                <option @if($item->sort_order == 2 )selected @endif   value="2">Специализированная программная аккредитация</option>
                                <option @if($item->sort_order == 3 )selected @endif  value="3">Специализированная программная аккредитация(ex-ante)</option>
                                <option @if($item->sort_order == 4 )selected @endif  value="4">    Институциональная первичная аккредитация (Ex-Ante) </option>
                                <option @if($item->sort_order == 5 )selected @endif  value="5">    Международная совместная аккредитация с агентством-партнером </option>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select  class="form-control"    name="country_id" id="country_id">
                                ...
                                @foreach(\App\Models\Country::where('is_accr',TRUE)->get() as $country)
                    ...
                    @if(!empty($item->id))
                        <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                            ...
                            @if(!($item->nodes()->where('lang','kz')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_accr_page_node',['lang' =>'kz','type' => $type,'parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                            @endif
                            @if(!($item->nodes()->where('lang','en')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_accr_page_node',['lang' =>'en','type' => $type,'parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                            @endif
                            <table class="table table-bordered">
                                ...
                                @foreach($item->nodes()->get() as  $node)
                                    <tr class="success">
                                        ...
                                        <td>{{$node->lang}}</td>
                                        <td><a href="{{route('edit_accr_page_node',['lang'=>$node->lang,'type' => $type,'parent' => $item->id ])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_node" data-toggle="modal" data-target="#delete-node-modal" itemname="{{$node->lang}}" itemid="{{$node->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
