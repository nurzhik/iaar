...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
            ...
            @else
                Редактирование страницы  переводов аккредитации "{{$parent->title}}"   на язык "{{$lang}}" типа учреждений ""{{array_flip(\App\Models\Univer::availableTypeIdArray())[$type]}}""
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/accreditation/{{$type}}/create" @else action="/admin/accreditation/{{$type}}/nodes/{{$parent->id}}/{{$lang}}" @endif>
            {{csrf_field()}}
