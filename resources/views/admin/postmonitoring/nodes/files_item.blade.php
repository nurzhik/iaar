...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(!empty($item->id))
                Редактирование перевода на язык "{{$lang}}" файла "{{$parent->title}}" 
            @else
                Добавление перевода на язык "{{$lang}}" файла "{{$parent->title}}" 
            @endif

        </h2>
        <script src="{{ asset("vendor/laravel-filemanager/js/stand-alone-button.js") }}"></script>
        <form method="post" @if(empty($item->id))action="/admin/postmonitoringfiles/nodes/create/{{$parent->id}}/{{$lang}}" @else action="/admin/postmonitoringfiles/nodes/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
