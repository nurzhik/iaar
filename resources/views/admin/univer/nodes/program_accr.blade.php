...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление перевода на язык "{{$lang}}" к  программной аккредитации
            @else
                Редактирования перевода на язык "{{$lang}}" к   программной аккредитации
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/univer/nodes/program_accr/{{$parent->id}}/{{$lang}}/create" @else action="/admin/univer/nodes/program_accr/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
