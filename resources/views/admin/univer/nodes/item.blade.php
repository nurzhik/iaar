...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление перевода на язык "{{$lang}}" к  университету
            @else
                Редактирования перевода на язык "{{$lang}}" к  университету
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/univer/nodes/univer/{{$parent->id}}/{{$lang}}/create" @else action="/admin/univer/nodes/univer/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
