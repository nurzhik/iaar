    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление события
            @else
                Редактирование события "{{$item->title}}"
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/articles/events/create" @else action="/admin/articles/events/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    <div role="tabpanel" class="tab-pane fade active in" id="main" aria-labelledby="main-tab">
                        ...
                        <div class="form-group">
                            <label for="dtp_input1" class=" control-label">ДАТА СОБЫТИЯ :</label>
                            <div class="input-group date form_datetime"  data-date-format="dd MM yyyy" data-link-field="event_date">
                                <input class="form-control" size="16" type="text" value="" readonly>
                                <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
                                <span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span>
                            </div>
                            <input type="hidden" id="event_date" name="event_date" value="{{$item->event_date}}" />
                        </div>
                        <div class="form-group">
