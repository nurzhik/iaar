...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление типа аккредитации DEQAR
            @else
                Редактирование типа аккредитации DEQAR
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/deqar_accr_types/create" @else action="/admin/deqar_accr_types/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    <div role="tabpanel" class="tab-pane fade active in" id="main" aria-labelledby="main-tab">
                        ...
                        <div class="form-group">
                            <label for="title">Значение поля:</label>
                            <input type="text" id="value" class="form-control"  value="{{$item->value}}" name="value" >
                        </div>
