...
@section('content')
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('.delete_node').on('click',function(){
                ...
                $('#delete_item_form').attr('action','/admin/naar/files/nodes/delete/'+item_id);
    ...
    <div class="blank-page">
        ...
        <form method="post" @if(empty($item->id))action="/admin/naar/files/create/{{$parent->id}}" @else action="/admin/naar/files/{{$parent->id}}/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
                    ...
                    @if(!empty($item->id))
                        <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                            ...
                            @if(!($item->nodes()->where('lang','kz')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('create_commisionfile_node',['lang' =>'kz','parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                            @endif
                            @if(!($item->nodes()->where('lang','en')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('create_commisionfile_node',['lang' =>'en','parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                            @endif
