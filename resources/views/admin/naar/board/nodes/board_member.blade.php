...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            ...
            @else
                Редактирования перевода на язык "{{$lang}}" к члену совета "{{$parent->title}}"
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/naar/board/nodes/board-member/{{$parent->id}}/{{$lang}}/create" @else action="/admin/naar/board/nodes/board-member/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
