...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление перевода на язык "{{$lang}}" к  "{{$parent->title}}"
            @else
                Редактирования перевода на язык "{{$lang}}" к "{{$parent->title}}"
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/naar/commisiontabs/nodes/{{$parent->id}}/{{$lang}}/create" @else action="/admin/naar/commisiontabs/nodes/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
