...
@section('content')
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('.delete_node').on('click',function(){
                ...
                $('#delete_item_form').attr('action','/admin/naar/commisiontabs/nodes/delete/'+item_id);
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление нового таба  для старинцы {{$parent->title}}
            @else
                Редактирование таба   старинцы {{$parent->title}}
            @endif
        </h2>
        <a href="/admin/naar/board/{{$parent->id}}" class="btn">
            Назад
        </a>
        <script src="{{ asset("vendor/laravel-filemanager/js/stand-alone-button.js") }}"></script>
        <form method="post" @if(empty($item->id))action="/admin/naar/commisiontabs/create/{{$parent->id}}" @else action="/admin/naar/commisiontabs/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
                    ...
                    @if(!empty($item->id))
                        <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                            ...
                            @if(!($item->nodes()->where('lang','kz')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('create_commisiontab_node',['lang' =>'kz','parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                            @endif
                            @if(!($item->nodes()->where('lang','en')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('create_commisiontab_node',['lang' =>'en','parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                            @endif
                            <table class="table table-bordered">
                                <thead>
                                <tr> <th>ID </th><th> Язык перевода</th><th> Заголовок</th> <th> ОПЕРАЦИИ </th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($item->nodes()->get() as  $node)
                                    <tr class="success">
                                        ...
                                        <td>{{$node->lang}}</td>
                                        <td><a href="{{route('edit_commisiontab_node',['lang'=>$node->lang,'item' => $node->id])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_node" data-toggle="modal" data-target="#delete-element-modal"  itemname="{{$node->lang}}" itemid="{{$node->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
