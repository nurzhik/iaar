...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
            ...
            Редактирование страницы "Международные сети"

        </h2>
        <form method="post" action="/admin/naar/int/intern_networks" >
            {{csrf_field()}}
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <span class="input-group-btn">
                                  <a  data-inputid="contact_face_photo" data-preview="contact_face_photo_preview" class="popup_selector btn btn-primary">
                    ...
                    @if(!empty($item->id))
                        <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                            ...
                            @if(!($item->nodes()->where('lang','kz')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_int_node',['slug' => 'intern_networks','lang' =>'kz'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                            @endif
                            @if(!($item->nodes()->where('lang','en')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_int_node',['slug' => 'intern_networks','lang' =>'en'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                            @endif
                            <table class="table table-bordered">
                                ...
                                @foreach($item->nodes()->get() as  $node)
                                    <tr class="success">
                                        ...
                                        <td>{{$node->lang}}</td>
                                        <td><a href="{{route('edit_int_node',['slug' => 'intern_networks','lang' =>$node->lang])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_node" data-toggle="modal" data-target="#delete-node-modal" itemname="{{$node->lang}}" itemid="{{$node->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
