...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
            ...
            Редактирование страницы "Международные проекты"

        </h2>
        <form method="post" action="/admin/naar/int/intern_projects" >
            {{csrf_field()}}
            <div class="blank-page">
                ...
                <ul id="myTabs" class="nav nav-tabs" role="tablist">
                    ...
                    <li role="presentation" class="">
                        <a href="#add_documents" id="add_documents-tab" role="tab" data-toggle="tab" aria-controls="add_documents" aria-expanded="false">Содержимое табов</a>
                    </li>
                    <li role="presentation" class="">
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    ...
                    <div role="tabpanel" class="tab-pane fade " id="add_documents" aria-labelledby="add_documents-tab">
                        <hr>
                        <h3>Содержимое табов</h3>
                        <hr>
                        <div id="tabs_c">
                            @if(strlen($item->add_documents))
                                <page-tabs-component ></page-tabs-component>
                            @else
                                <page-tabs-component ></page-tabs-component>
                            @endif
                        </div>
                    ...
                    @if(!empty($item->id))
                        <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                            ...
                            @if(!($item->nodes()->where('lang','kz')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_int_node',['slug' => 'intern_projects','lang' =>'kz'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                            @endif
                            @if(!($item->nodes()->where('lang','en')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_int_node',['slug' => 'intern_projects','lang' =>'en'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                            @endif
                            <table class="table table-bordered">
                                ...
                                @foreach($item->nodes()->get() as  $node)
                                    <tr class="success">
                                        ...
                                        <td>{{$node->lang}}</td>
                                        <td><a href="{{route('edit_int_node',['slug' => 'intern_projects','lang' =>$node->lang])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_node" data-toggle="modal" data-target="#delete-node-modal" itemname="{{$node->lang}}" itemid="{{$node->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
