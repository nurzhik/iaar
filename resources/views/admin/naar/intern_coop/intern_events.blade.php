...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
            ...
            Редактирование страницы "Международные мероприятия"

        </h2>
        <form method="post" action="/admin/naar/int/intern_events" >
            {{csrf_field()}}
            <div class="blank-page">
                ...
                <ul id="myTabs" class="nav nav-tabs" role="tablist">
                    ...
                    <li role="presentation" class="">
                        <a href="#events" id="documents-tab" role="tab" data-toggle="tab" aria-controls="events" aria-expanded="false">СПИСОК МЕРОПРИЯТИЙ</a>
                    </li>
                    <li role="presentation" class="">
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    ...
                    <div role="tabpanel" class="tab-pane fade " id="events" aria-labelledby="events-tab">
                        ...
                        <hr>
                        <a style="display:block;margin-bottom:22px;" href="{{route('create_int_event')}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добаваить мероприятие </button></a>
                        <table class="table table-bordered">
                            <thead>
                            <tr> <th>ID </th><th> Заголовок</th> <th> Дата публикации</th>  <th> ОПЕРАЦИИ </th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach(\App\Models\StaticPage::where('type_id',24)->orderBy('created_at','DESC')->get() as $event)
                                <tr class="success">
                                    <th scope="row">{{$event->id}}</th>
                                    <td>{{$event->title}}</td>
                                    <td>{{date('d.m.Y',strtotime($event->created_at))}}</td>
                                    <td><a href="{{route('edit_int_event',['item'=>$event->id])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_event" data-toggle="modal" data-target="#delete-event-modal" itemname="{{$event->title}}" itemid="{{$event->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
                    ...
                    <div role="tabpanel" class="tab-pane fade " id="contact_face" aria-labelledby="contact_face-tab">
                        ...
                        <hr>
                        <?php $contact_face = $item->teamMembers()->firstOrNew([]); ?>
                        ...
                        <div class="form-group">
                            <label for="contact_face_email">Email контактного лица</label>
                            <input type="text" value="{{$contact_face->email}}" class="form-control" name="contact_face_email" id="contact_face_email">
                        </div>
                        <div class="form-group">
                            <label for="contact_face_photo">Фотография контактного лица</label>
                            <div class="input-group">
                                <span class="input-group-btn">
                                  <a data-inputid="contact_face_photo" data-preview="contact_face_photo_preview" class="btn btn-primary popup_selector">
                                     <i class="fa fa-picture-o"></i> Выберите картинку
                                    </a>
                                </span>
                                <input readonly="readonly" id="contact_face_photo" class="form-control" type="text" value="{{$contact_face->photo}}" name="contact_face_photo">
                            </div>
                            <span id="contact_face_photo_preview" style="margin-top:15px;">
                                @if(isset($contact_face->photo))
                                    <img src="{{$contact_face->photo}}" class="preview-icon">
                                @endif
                            </span>
                        </div>
                        <button type="submit" class="submit_button btn btn-success">СОХРАНИТЬ</button>
                    ...
                    @if(!empty($item->id))
                        <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                            ...
                            @if(!($item->nodes()->where('lang','kz')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_int_node',['slug' => 'intern_events','lang' =>'kz'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                            @endif
                            @if(!($item->nodes()->where('lang','en')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_int_node',['slug' => 'intern_events','lang' =>'en'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                            @endif
                            <table class="table table-bordered">
                                <thead>
                                ...
                                @foreach($item->nodes()->get() as  $node)
                                    <tr class="success">
                                        ...
                                        <td>{{$node->lang}}</td>
                                        <td><a href="{{route('edit_int_node',['slug' => 'intern_events','lang' =>$node->lang])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_node" data-toggle="modal" data-target="#delete-node-modal" itemname="{{$node->lang}}" itemid="{{$node->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="delete_event_title">Удаление бренда</h4>
                </div>
                <div class="modal-body">
                    <form  action="задается скриптом" method="POST" id="delete_event_form">
                        {{ csrf_field() }}
                        <div class="form-group">
                            Удалить данное мероприятие ?
                        </div>
