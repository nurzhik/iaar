...
@section('content')
    ...
    <div class="main-page">
        ...
        <form method="post" action="/admin/naar/team">
            {{csrf_field()}}
            <div class="blank-page">
                ...
                <ul id="myTabs" class="nav nav-tabs" role="tablist">
                    ...
                    <li role="presentation" >
                        <a href="#team" id="documents-tab" role="tab" data-toggle="tab" aria-controls="documents" aria-expanded="false">Члены нашей команды</a>
                    </li>
                    <li role="presentation" >
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    ...
                    <div role="tabpanel" class="tab-pane fade" id="team" aria-labelledby="team-tab">
                        ...
                        <hr>
                        <a style="display:block;margin-bottom:22px;" href="{{route('create_team_member')}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить члена команды</button></a>
                        <table class="table table-bordered">
                            <thead>
                            <tr> <th>ID </th><th>ПОРЯДОК СОРТИРОВКИ</th> <th>ФОТО</th> <th>ИМЯ ФАМИЛИЯ </th> <th>ДОЛЖНОСТЬ </th> <th> ОПЕРАЦИИ </th>                            </tr>
                            </thead>
                            <tbody>
                            @foreach($item->teamMembers()->orderBy('sort_order')->get() as $member)
                                <tr class="success">
                                    ...
                                    <td>{{$member->name}}</td>
                                    <td>{{$member->job}}</td>
                                    <td><a href="{{route('edit_team_member',['item'=>$member->id])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_category" data-toggle="modal" data-target="#delete-element-modal" itemname="{{$member->title}}" itemid="{{$member->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
                    ...
                    <div role="tabpanel" class="tab-pane fade " id="translations" aria-labelledby="translations-tab">
                        <hr>
                        <h2>Список переводов</h2>
                        <hr>
                        @if(!($item->nodes()->where('lang','kz')->exists()))
                            <a style="display:block;margin-bottom:22px;" href="{{route('edit_team_page_node',['lang' =>'kz'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                        @endif
                        @if(!($item->nodes()->where('lang','en')->exists()))
                            <a style="display:block;margin-bottom:22px;" href="{{route('edit_team_page_node',['lang' =>'en'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                        @endif
                        <table class="table table-bordered">
                            <thead>
                            ...
                            @foreach($item->nodes()->get() as  $node)
                                <tr class="success">
                                    ...
                                    <td>{{$node->lang}}</td>
                                    <td><a href="{{route('edit_team_page_node',['lang' =>$node->lang])}}"><i class="glyphicon glyphicon-edit"></i></a></td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
