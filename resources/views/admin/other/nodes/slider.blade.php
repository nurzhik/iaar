...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление перевода на язык "{{$lang}}" к  слайдеру "{{$parent->title}}"
            @else
                Редактирования перевода на язык "{{$lang}}" к слайдеру "{{$parent->title}}"
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/other/nodes/sliders/{{$parent->id}}/{{$lang}}/create" @else action="/admin/other/nodes/sliders/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
