...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление нового "признание"
            @else
                Редактирование "признание"
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/other/acceptance_partners/create" @else action="/admin/other/acceptance_partners/{{$item->id}}" @endif>
            {{csrf_field()}}
