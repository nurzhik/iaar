...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if($create)
                Добавление перевода на язык '{{$lang}}'
            @else
                Редактирование перевода на язык '{{$lang}}'
            @endif
        </h2>
        <form method="post" action="/admin/requests/forms/node/{{$lang}}@if($create)/create{{''}}@endif">
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
                            ...
                            <div class="form-group">
                                ...
                                <span class="input-group-btn">
                                  <a data-inputid="file_{{$value}}" class="btn btn-primary popup_selector">
                                    ...
                                    <input readonly="readonly" id="file_{{$value}}" class="form-control" type="text" value="{{\App\Models\AccrRequestForm::where('type_id',$value)->first()->nodes()->where('lang',$lang)->firstOrNew([])->file}}" name="file[{{$value}}]">
                                </div>
                            </div>
                        @endforeach
