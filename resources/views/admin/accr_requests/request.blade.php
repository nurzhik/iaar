...
@section('content')
    ...
    <div class="blank-page">
        ...
        <form method="post" action="/admin/requests/{{$item->id}}">
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    <div role="tabpanel" class="tab-pane fade active in" id="main" aria-labelledby="main-tab">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" readonly="readonly" id="name" class="form-control"  value="{{$item->name}}" name="name" >
                        </div>
                        <div class="form-group">
                            <label for="email">Email:</label>
                            <input type="text" readonly="readonly" id="email" class="form-control"  value="{{$item->email}}" name="email" >
                        </div>
                        <div class="form-group">
                            <label for="message">Примечание:</label>
                            <input type="text" readonly="readonly" id="message" class="form-control"  value="{{$item->message}}" name="message" >
                        </div>
                        <div class="form-group">
                            <label for="file">Ссылка на документ:</label>
                            <a href="{{$item->file}}" target="_blank">{{$item->file}}</a>
                        </div>

                        <div class="form-group">
                            <label for="published">Просмотрено?</label>
                            <input type="checkbox"   class="tgl tgl-light" {{$item->viewed ? 'checked' : ''}} name="viewed" id="viewed">
                            <label class="tgl-btn"   for="viewed"></label>
                        </div>
