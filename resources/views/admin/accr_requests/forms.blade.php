...
@section('content')
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $('.delete_node').on('click',function(){
                ...
                $('#delete_node_form').attr('action','/admin/requests/forms/node/delete/'+item_id);
    ...
    <div class="blank-page">
        <h2 class="title1">
            Редактирование списка образцов заявок на аккредитацию
        </h2>
        <form method="post" action="/admin/requests/forms">
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    <div role="tabpanel" class="tab-pane fade active in" id="main" aria-labelledby="main-tab">
                        ...
                        <div class="form-group">
                            <label for="file">Документ для "{{$key}}"</label>
                            <div class="input-group">
                                <span class="input-group-btn">
                                  <a data-inputid="file_{{$value}}" class="btn btn-primary popup_selector">
                                     <i class="fa fa-picture-o"></i> Выберите документ
                                  </a>
                                </span>
                                <input readonly="readonly" id="file_{{$value}}" class="form-control" type="text" value="{{\App\Models\AccrRequestForm::where('type_id',$value)->firstOrNew(['type_id' => $value])->file}}" name="file[{{$value}}]">
                            </div>
                        </div>
                        @endforeach
                        ...
                        <script>
                            $(document).ready(function(){
                                @foreach(\App\Models\Univer::availableTypeIdArray() as $key=>$value)
                                $('#file_{{$value}}_picker').filemanager('image');
                                @endforeach
                            });
                        </script>
                    ...
                    <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                        ...
                        <hr>
                        @if(!empty($check))
                        @if(!in_array('kz',$existing_langs))
                        <a style="display:block;margin-bottom:22px;" href="{{route('create_request_form_node',['lang' =>'kz'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                        @endif
                        @if(!in_array('en',$existing_langs))
                        <a style="display:block;margin-bottom:22px;"href="{{route('create_request_form_node',['lang' =>'en'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык  </button></a>
                        @endif
                        <table class="table table-bordered">
                            ...
                            @foreach($existing_langs as $lang)
                                <tr class="success">
                                    ...
                                    <td>{{$lang}}</td>
                                    <td><a href="{{route('edit_request_form_node',['lang'=>$lang])}}"><i class="glyphicon glyphicon-edit"></i></a><!--<a class="delete_node" data-toggle="modal" data-target="#delete-node-modal" itemname="{{$lang}}" itemid="{{$lang}}" href="#"><i class="glyphicon glyphicon-remove"></i></a>--></td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
