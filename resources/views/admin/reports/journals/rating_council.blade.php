...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
            Редактирование страницы  консультационного совета Рейтинга
        </h2>
        <form method="post" action="/admin/rating/rating_council">
            {{csrf_field()}}
            <div class="blank-page">
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    <div role="tabpanel" class="tab-pane fade active in" id="main" aria-labelledby="main-tab">
                        ...
                        <textarea name="text" id="text" class="form-control my-editor">{!! $item->content !!}</textarea>
                        <script>
                          var editor_config = {
                            path_absolute : "/",
                    ...
                    @if(!empty($item->id))
                        <div role="tabpanel" class="tab-pane fade" id="team" aria-labelledby="team-tab">
                            ...
                            Члены совета
                            <hr>
                            <a style="display:block;margin-bottom:22px;" href="{{route('create_board_member_rating')}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить члена совета</button></a>
                            <table class="table table-bordered">
                                ...
                                @foreach($item->boardMembers()->orderBy('sort_order')->get() as $member)
                                    <tr class="success">
                                        ...
                                        <td>{{$member->title}}</td>
                                        <td>{{$member->job}}</td>
                                        <td><a href="{{route('edit_board_member_rating',['item'=>$member->id])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_category" data-toggle="modal" data-target="#delete-element-modal" itemname="{{$member->title}}" itemid="{{$member->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
                    ...
                    <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                        ...
                        @if(!($item->nodes()->where('lang','kz')->exists()))
                            <a style="display:block;margin-bottom:22px;" href="{{route('edit_rating_council_node',['lang' =>'kz'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                        @endif
                        @if(!($item->nodes()->where('lang','en')->exists()))
                            <a style="display:block;margin-bottom:22px;" href="{{route('edit_rating_council_node',['lang' =>'en'])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                        @endif
                        <table class="table table-bordered">
                            <thead>
                            ...
                            @foreach($item->nodes()->get() as  $node)
                                <tr class="success">
                                    ...
                                    <td>{{$node->lang}}</td>
                                    <td><a href="{{route('edit_rating_council_node',['lang'=>$node->lang])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_node" data-toggle="modal" data-target="#delete-node-modal" itemname="{{$node->lang}}" itemid="{{$node->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
