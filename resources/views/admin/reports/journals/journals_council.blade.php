...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
                Редактирование страницы  Редакционного Совета Журнала Education QA
        </h2>
        <form method="post" action="/admin/reports/journal/journals_council">
            {{csrf_field()}}
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    ...
                    @if(!empty($item->id))
                        <div role="tabpanel" class="tab-pane fade" id="team" aria-labelledby="team-tab">
                            ...
                            Члены совета
                            <hr>
                            <a style="display:block;margin-bottom:22px;" href="{{route('create_board_member_journal')}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить члена совета</button></a>
                            <table class="table table-bordered">
                                ...
                                @foreach($item->boardMembers()->orderBy('sort_order')->get() as $member)
                                    <tr class="success">
                                        ...
                                        <td>{{$member->title}}</td>
                                        <td>{{$member->job}}</td>
                                        <td><a href="{{route('edit_board_member_journal',['item'=>$member->id])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_category" data-toggle="modal" data-target="#delete-element-modal" itemname="{{$member->title}}" itemid="{{$member->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
