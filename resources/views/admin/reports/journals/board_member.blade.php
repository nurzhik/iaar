...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
            @if(empty($item->id)) Добавление члена редакционного совета @else Редактирование члена редакционного совета @endif
        </h2>
        <form method="post"  @if(empty($item->id))action="/admin/reports/journal/board-member/create" @else action="/admin/reports/journal/board-member/{{$item->id}}" @endif>
            {{csrf_field()}}
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                    <span class="input-group-btn">
                                        <a data-inputid="main_image" data-preview="maini_mage_preview" class="btn btn-primary popup_selector">
                    ...
                    <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                        ...
                        @if(!($item->nodes()->where('lang','kz')->exists()))
                            <a style="display:block;margin-bottom:22px;" href="{{route('edit_board_member_journal_node',['lang' =>'kz','parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                        @endif
                        @if(!($item->nodes()->where('lang','en')->exists()))
                            <a style="display:block;margin-bottom:22px;" href="{{route('edit_board_member_journal_node',['lang' =>'en','parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                        @endif
                        <table class="table table-bordered">
                            ...
                            @foreach($item->nodes()->get() as  $node)
                                <tr class="success">
                                    ...
                                    <td>{{$node->lang}}</td>
                                    <td><a href="{{route('edit_board_member_journal_node',['lang' =>$node->lang,'parent' => $item->id])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_node" data-toggle="modal" data-target="#delete-node-modal" itemname="{{$node->lang}}" itemid="{{$node->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
