...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление перевода на язык "{{$lang}}" к  члену совета "{{$parent->title}}"
            @else
                Редактирование перевода на язык "{{$lang}}" к члену совета "{{$parent->title}}"
            @endif
        </h2>
        <form method="post"   action="/admin/reports/journal/nodes/board-member/{{$parent->id}}/{{$lang}}" >
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    <div role="tabpanel" class="tab-pane fade active in" id="main" aria-labelledby="main-tab">
                        ...
                        <textarea name="text" id="text" class="form-control my-editor text-editor">{!! $item->content !!}</textarea>
                        <script src="{{ asset("vendor/laravel-filemanager/js/stand-alone-button.js") }}"></script>
                        <script>
                            $(document).ready(function(){
