...
@section('content')
    ...
    <div class="main-page">
        <h2 class="title1">
            @if(empty($item->id)) Добавление члена консультационного совета @else Редактирование члена консультационного совета @endif
        </h2>
        <form method="post"  @if(empty($item->id))action="/admin/rating/board-member/create" @else action="/admin/rating/board-member/{{$item->id}}" @endif>
            {{csrf_field()}}
                            ...
                            <table class="table table-bordered">
                                ...
                                @foreach($item->nodes()->get() as  $node)
                                    <tr class="success">
                                        ...
                                        <td>{{$node->lang}}</td>
                                        <td><a href="{{route('edit_board_member_journal_node',['item'=>$node->id])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_node" data-toggle="modal" data-target="#delete-node-modal" itemname="{{$node->lang}}" itemid="{{$node->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
