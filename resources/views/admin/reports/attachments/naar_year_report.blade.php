...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление ежегодного отчета к странице "Ежегодные отчеты НААР"
            @else
                Редактирование ежегодного отчета  страницы "Ежегодные отчеты НААР"
            @endif
        </h2>
        <script src="{{ asset("vendor/laravel-filemanager/js/stand-alone-button.js") }}"></script>
        <form method="post" @if(empty($item->id))action="/admin/reports/11/attachment/{{$parent->id}}/create" @else action="/admin/reports/11/attachment/{{$parent->id}}/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
