...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
                @if(!empty($item->id))
                Редактирование перевода на язык "{{$lang}}" ежегодного отчета "{{$parent->title}}"  страницы "Ежегодные отчеты НААР"
                    @else
                Добавление перевода на язык "{{$lang}}" ежегодного отчета "{{$parent->title}}"  страницы "Ежегодные отчеты НААР"
            @endif
