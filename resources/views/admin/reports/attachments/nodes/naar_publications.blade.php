...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(!empty($item->id))
                Редактирование перевода на язык "{{$lang}}" документа "{{$parent->title}}"  к странице "Публикации и отчеты"
            @else
                Добавление перевода на язык "{{$lang}}" документа "{{$parent->title}}"  страницы "Публикации и отчеты"
            @endif
