...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(!empty($item->id))
                Редактирование перевода на язык "{{$lang}}" видео "{{$parent->title}}"  к странице "Видеоархивы"
            @else
                Добавление перевода на язык "{{$lang}}" видео "{{$parent->title}}"  страницы "Видеоархивы"
            @endif

        </h2>
        <script src="{{ asset("vendor/laravel-filemanager/js/stand-alone-button.js") }}"></script>
        <form method="post" @if(empty($item->id))action="/admin/reports/nodes/attachment/create/{{$parent->id}}/{{$lang}}" @else action="/admin/reports/nodes/attachment/{{$item->id}}/{{$lang}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
