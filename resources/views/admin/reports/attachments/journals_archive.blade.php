...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление журнала к странице "Архивы журнала EDUCATION QA"
            @else
                Редактирование журнала  страницы "Архивы журнала EDUCATION QA"
            @endif
        </h2>
        <script src="{{ asset("vendor/laravel-filemanager/js/stand-alone-button.js") }}"></script>
        <form method="post" @if(empty($item->id))action="/admin/reports/13/attachment/{{$parent->id}}/create" @else action="/admin/reports/13/attachment/{{$parent->id}}/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
