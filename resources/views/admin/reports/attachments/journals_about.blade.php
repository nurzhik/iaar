...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление журнала к странице "О Журнале"
            @else
                Редактирование журнала  страницы "О Журнале"
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/reports/29/attachment/{{$parent->id}}/create" @else action="/admin/reports/29/attachment/{{$parent->id}}/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
