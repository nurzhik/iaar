...
@section('content')
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление документа к странице "Публикации и отчеты"
            @else
                Редактирование документа  страницы "Публикации и отчеты"
            @endif
        </h2>
        <script src="{{ asset("vendor/laravel-filemanager/js/stand-alone-button.js") }}"></script>
        <form method="post" @if(empty($item->id))action="/admin/reports/14/attachment/{{$parent->id}}/create" @else action="/admin/reports/14/attachment/{{$parent->id}}/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
