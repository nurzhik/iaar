...
@section('content')
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $('.delete_node').on('click',function(){
                ...
                $('#delete_node_form').attr('action','/admin/naar/int/events/nodes/delete/'+item_id);
    ...
    <div class="blank-page">
        <h2 class="title1">
            @if(empty($item->id))
                Добавление события СМИ
            @else
                Редактирование  события СМИ "{{$item->title}}"
            @endif
        </h2>
        <form method="post" @if(empty($item->id))action="/admin/reports/events/create" @else action="/admin/reports/events/{{$item->id}}" @endif>
            {{csrf_field()}}
            <div class="blank-page">
                @if (session('status'))
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    <div role="tabpanel" class="tab-pane fade active in" id="main" aria-labelledby="main-tab">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="short_desc" id="short_desc" class="form-control ">{{$item->additional_documents}}</textarea>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="hidden" id="dtp_input1" name="published_at" value="{{$item->created_at}}" />
                        </div>
                        <div class="form-group">
                    ...
                    @if(!empty($item->id))
                        <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                            ...
                            @if(!($item->nodes()->where('lang','kz')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('create_smi_event_node',['lang' =>'kz','parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                            @endif
                            @if(!($item->nodes()->where('lang','en')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('create_smi_event_node',['lang' =>'en','parent' => $item->id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                            @endif
                            <table class="table table-bordered">
                                ...
                                @foreach($item->nodes()->get() as  $node)
                                    <tr class="success">
                                        ...
                                        <td>{{$node->lang}}</td>
                                        <td><a href="{{route('edit_smi_event_node',['item'=>$node->id])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_node" data-toggle="modal" data-target="#delete-node-modal" itemname="{{$node->lang}}" itemid="{{$node->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
