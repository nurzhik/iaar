...
@section('content')
    <script type="text/javascript">
        $(document).ready(function(){
            $('.delete_main_accr').on('click',function(){
                ...
                $('#delete_attachment_form').attr('action','/admin/reports/14/attachment/{{$item->id}}/delete/'+item_id);
    ...
    <div class="main-page">
        <h2 class="title">
            Редактирование страницы публикаций НААР
        </h2>
        <form method="post" action="/admin/reports/14/edit" >
            {{csrf_field()}}
            <div class="blank-page">
                ...
                <ul id="myTabs" class="nav nav-tabs" role="tablist">
                    ...
                    <li role="presentation" class="">
                        <a href="#add_documents" id="add_documents-tab" role="tab" data-toggle="tab" aria-controls="add_documents" aria-expanded="false">Список публикаций</a>
                    </li>
                    <li role="presentation" class="">
                ...
                <div id="myTabContent" class="tab-content scrollbar1">
                    ...
                    <div role="tabpanel" class="tab-pane fade " id="add_documents" aria-labelledby="add_documents-tab">
                        ...
                        <hr>
                        <div >
                            <a style="display:block;margin-bottom:22px;" href="{{route('create_attachment',['parent' => $item,'type' => 14])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить вложение </button></a>
                            <div class="blank-page widget-shadow scroll">
                                <table class="table table-bordered">
                                    <thead>
                                    <tr>  <th>Год </th> <th>Порядок сортировки </th>Картинка превью<th>Заголовок</th> <th>Краткое описание</th> <th>ссылка на файл</th> <th> ОПЕРАЦИИ </th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach($item->attachments()->orderBy('year','DESC')->orderBy('sort_order')->get() as $attachment)
                                        <tr class="success">
                                            <th scope="row">{{$attachment->year}}</th>
                                            <td>{{$attachment->sort_order}}</td>
                                            <td><img src="{{$attachment->image}}" style="height:20px;"></td>
                                            <td>{{$attachment->title}}</td>
                                            <td>{{$attachment->short_desc}}</td>
                                            <td><a href="{{route('edit_attachment',['parent' => $item,'item'=>$attachment->id,'type'=>14])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_main_accr" data-toggle="modal" data-target="#delete-element-modal"  itemid="{{$attachment->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
                    ...
                    @if(!empty($item->id))
                        <div role="tabpanel" class="tab-pane fade in" id="translations" aria-labelledby="translations-tab">
                            ...
                            @if(!($item->nodes()->where('lang','kz')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_reports_page_node',['lang' =>'kz','type' => $item->type_id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на казахский язык </button></a>
                            @endif
                            @if(!($item->nodes()->where('lang','en')->exists()))
                                <a style="display:block;margin-bottom:22px;" href="{{route('edit_reports_page_node',['lang' =>'en','type' => $item->type_id])}}"> <button type="button" class="btn btn-pri btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Добавить перевод на английский язык </button></a>
                            @endif
                            <table class="table table-bordered">
                                <thead>
                                ...
                                @foreach($item->nodes()->get() as  $node)
                                    <tr class="success">
                                        ...
                                        <td>{{$node->lang}}</td>
                                        <td><a href="{{route('edit_reports_page_node',['lang'=>$node->lang,'type' => $item->type_id])}}"><i class="glyphicon glyphicon-edit"></i></a><a class="delete_node" data-toggle="modal" data-target="#delete-node-modal" itemname="{{$node->lang}}" itemid="{{$node->id}}" href="#"><i class="glyphicon glyphicon-remove"></i></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
