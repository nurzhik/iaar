...
class AttachmentsNodesController extends Controller
{
    public function create( Attachment $parent, $lang)
    {
        ...
        return view('admin.articles.nodes.attachment',[
            'item' =>$item,
            'parent' => $parent,
            'lang' => $lang,
        ]);
    ...
    public function store( Request $request, Attachment $parent, $lang)
    {
        ...
        $item->lang = $lang;
        $item->save();
        return redirect()->route('edit_attachment_article',['parent' => $parent->article_id,'item' => $parent->id])->with('status','Новый перевод успешно добавлен!');
    }
    ...
    public function update(AttachmentNode $item, $lang, Request $request)
    {
        ...
        $item->lang = $lang;
        $item->save();
        return redirect()->route('edit_attachment_article',['parent' => $parent->article_id,'item' => $parent->id])->with('status','Данные о переводе успешно изменены');
    }
