...
class SmiEventsNodesController extends Controller
{

    public function create( StaticPage $parent, $lang)
    {
        if($parent->type_id!=30)
        ...
        return view('admin.reports.nodes.smi_event',[
            'item' => $item,
            'lang' => $lang,
            'parent' => $parent
        ]);
    ...
    public function store( StaticPage $parent, $lang, Request $request)
    {
        ...
        $item->lang = $lang;
        $item->save();
        return redirect()->route('edit_smi_event',['item' => $parent->id])->with('status','Новый перевод успешно создан!');
    ...
    public function update(StaticPageNode $item, Request $request)
    {
        ...
        $item->save();
        $parent = StaticPage::where('id',$item->parent_id)->first();
        return redirect()->route('edit_smi_event',['item' => $parent->id])->with('status',' перевод успешно изменен!');
    }
