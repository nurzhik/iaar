...
class InternationalEventsController extends Controller
{
    ...
    {
        $item = new StaticPage();
        return view('admin.naar.intern_coop.event',[
            'item' => $item,
        ]);
    }
    public function store(Request $request)
    {
        ...
        $item->save();
        return redirect()->route('edit_int_page',['slug'=>'intern_events'])->with('status','Новая запись успешно создана!');
    ...
    public function update(StaticPage $item, Request $request)
    {
        ...
        $item->seo_description = $request->seo_description;
        $item->save();
        return redirect()->route('edit_int_page',['slug'=>'intern_events'])->with('status','Данные о записи успешно изменены!');
